function yasfetch --description 'a simple fetch for fedora and gnome'
	...
	set -l uptime (uptime)
	set -l resolution (xdpyinfo | awk '/dimensions:/ { print $2 } ')
	...
	set logo[8] $logo[8]$lc"re $ic• $resolution"
	set logo[9] $logo[9]$lc"up $ic• $uptime"
	set logo[10] $logo[10]$lc" $c6󰮯 • $c5󰊠  $c3󰊠  $c1󰊠  $c7󰊠"
